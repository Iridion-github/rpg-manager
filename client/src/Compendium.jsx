 ...
 * is saved, nothing is anybody's, and every player sees the same thing.
 *
 * ## How it walks
 *
 ...
 *
 * ## Reading it, and taking from it
 *
 * The Items tab renders this with no arguments at all: the whole shelf, to be
 * read. Given `onUse`, an entry grows a button that hands the document to
 * whoever opened it, which is how a piece of armour on a character sheet gets
 * filled in from the book rather than typed. Given `only`, the categories are
 * cut to the ones named - a sheet asking for armour has no business offering
...
export default function Compendium({
  ...
  // Null for the lot.
  only = null,
  // Called with the raw document when the reader presses the button on an
  ...
  // A list, not a Set, so a caller can write it inline without it counting as a
  // new value on every render. Compared by content below for the same reason.
  ...
  const loadCategories = useCallback(async () => {
    ...
    setError('');
    try {
      // Through branchesOf like every other level of the walk, which is also
      // what puts them in alphabetical order. Filtering keeps that order, so a
      // narrowed list reads the same way as the whole shelf.
      const all = branchesOf(await fetchNode(ROOT));
      const wanted = onlyKey ? onlyKey.split(',') : null;
      setCategories(wanted ? all.filter((c) => wanted.includes(c.index)) : all);
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="items-view">
      ...
      {!loading && !error && entry && (
        <Entry node={entry} onUse={onUse} useLabel={useLabel} />
      )}
 ...
 *
 * The button at the top is there only when somebody is shopping rather than
 * reading. At the top rather than at the bottom because a magic item runs to
 * four paragraphs, and the button belongs beside the name it applies to.
 */
function Entry({ node, onUse, useLabel }) {
  ...
  return (
    <article className="items-entry">
      <div className="items-entry-head">
        <h3>{node.name}</h3>
        {onUse && (
          <button type="button" className="items-use" onClick={() => onUse(node)}>
            {useLabel}
          </button>
        )}
      </div>
